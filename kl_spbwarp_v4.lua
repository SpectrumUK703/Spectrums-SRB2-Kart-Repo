...
rawset(_G, "spbwarp" , CV_RegisterVar({
	name = "spbwarp",
	defaultvalue = "On",
	flags = CV_NETVAR,
...
local function BlueShell(actor, mo)
	...
	if spbwarp.value
		if actor.tracer and actor.tracer.player
			...
			local spbDist = (FixedDiv(P_AproxDistance(actor.tracer.player.mo.x - actor.x, actor.tracer.player.mo.y - actor.y) - actor.radius, actor.tracer.player.mo.scale)) / FRACUNIT
