...
local function shouldTumbleFinishedPlayer(player, minimumIgnorance)
	return player.exiting == 0 or cv_tumbleIgnoresFinish.value >= minimumIgnorance
end
...
local function tumbleGeneralCheck(player, ignorance)
	...
	return not (
		...
		and shouldTumbleFinishedPlayer(player, ignorance)
...
local function canTumbleForSpin(player, inflictor, source)
	if shouldTumbleFinishedPlayer(player, TUMBLE_IGNORE_FINISH_TYPE.Vanilla) then
		tumblePlayer(player, inflictor, source)
	end
end
