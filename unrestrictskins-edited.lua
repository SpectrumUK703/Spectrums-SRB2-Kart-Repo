...
local cv_unrestrictskinsposition = CV_RegisterVar({
	...
	flags = CV_NETVAR,
	description = "Allow changing skins during position, and the first 20 seconds of the race.",
})
...
addHook("TeamSwitch", function(p, newteam, fromspectators, tryingautobalance, tryingscramble)
	if (cv_unrestrictskins.value or (cv_unrestrictskinsposition.value and (leveltime <= starttime + 20*TICRATE))) and newteam == 0 and not (fromspectators or tryingautobalance or tryingscramble) then
