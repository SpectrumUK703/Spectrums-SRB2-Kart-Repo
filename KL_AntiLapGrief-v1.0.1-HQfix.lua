...
local function NoLapGrief(p, i, s)
	...
	if i and i.valid and i.player and i.player.valid
		if s and s.valid and s.player and s.player.valid
			if s.player.kartstuff[k_invincibilitytimer] > 0
			or s.player.kartstuff[k_growshrinktimer] > 0
			or (HugeQuest and s.player.hugequest and s.player.hugequest.huge > 0) then
				p.laps = $ or 0
				s.player.laps = $ or 0
...
addHook("ShouldDamage", function(mo, i, s, d)
	...
	if d < 10000 then
		if (i and i.valid and i.player and i.player.valid) then
			if s and s.valid and s.player and s.player.valid then
