...
local function shouldEatIFramesAtAll(player)
	-- check for modded iframe conditions here since we can't rely on vanilla flow
    return cv_tumbleMemeEnabled.value and cv_tumbleIgnoresIFrames.value and tumbleGeneralCheckModded(player)
end

local function eatIFrames(player, inflictor, source)
    --we don't want to return false (prevent damage) if this doesn't pass
    if shouldEatIFramesAtAll(player) and tumbleGeneralCheck(player) then
        return true
    end
end

local function canEatIFramesForSquish(player, inflictor, source)
    if shouldEatIFramesAtAll(player) and tumbleSquishCheck(inflictor, source) then
        return true
    end
end
...
local function eatIFramesForDamage(target, inflictor, source, damage)
    ...
    if player and player.valid and not player.spectator and not player.exiting and shouldEatIFramesAtAll(player) then
